...
c                                                         c  
c            Time-stamp: <2012-09-13 14:04:35 ataruya>    c
c ******************************************************* c
c
c     Note--. 
c
...
c
      ...
      real*8   pi, Lbox, zred, ss, sigmav, growth, dd2, f
...
c
c     ///////  RegPT 1-loop calculation  ///////
c
      write(6,*) ' Now computing RegPT 1-loop power spectra '
c
      call calc_regpt_1loop(zred)
c
...
c     /////// Save output data ///////
c
cc      open(10,file='corr_Bterm_RegPT.dat',status='unknown')
cc      open(11,file='pkcorr_Bterm_RegPT.dat',status='unknown')
c
cc      do ik =1, ik_max
cc         write(10,'(1p8e18.10)') ak(ik), pk(ik), 
cc     &        pk0EH(ik), pk0corr(ik), pk2EH(ik), 
cc     &        pk2corr(ik), pk4EH(ik), pk4corr(ik) 
cc         write(11,'(1p5e18.10)') ak(ik), pk_B1(ik), pk_B2(ik), 
cc     &        pk_B3(ik), pk_B4(ik)
cc      enddo
c
cc      close(10)
cc      close(11)
c
cc      write(6,*) '---> output files:'
cc      write(6,*) '         corr_pkred_RegPT.dat'
cc      write(6,*) '         pkcorr_red_RegPT.dat'
c
      end
c
c ******************************************************* c
...
c ******************************************************* c
c
      subroutine load_RegPT_data
c
c ******************************************************* c
c
c     input file is assumed to be the RegPT 1-loop power spectrum data 
c
      implicit none
c
      integer ik, ikk, ikmax, ik_max
      ...
      real*8  pk_EH, pk_lin, pi
      common /pk_RegPT_data/ ak, pk_dd, pk_dt, pk_tt, ik_max
      character infile*50
      pi = 4.d0 * datan(1.d0)
c     --------------------------------------------------------
c
      write(6,*) ' Type file name of RegPT 1-loop data'
      read(5,*) infile
      open(9, file=infile, status='unknown')
c
      do ik=1, ikmax
         read(9,*,END=10) ak(ik), pk_EH, pk_lin, pk_dd(ik), 
     &        pk_dt(ik), pk_tt(ik)
      enddo
c
 10   continue
      ik_max = ik - 1
c
      write(6,*) 'ak_RegPT(1)=', ak(1)
      write(6,*) 'ak_RegPT(ik_max)=', ak(ik_max)
      write(6,*) 'ik_max=', ik_max
      write(6,*)
c
      close(9)
...
c ******************************************************* c
c
      subroutine calc_regpt_1loop(zred)
c
c ******************************************************* c
c
      implicit none
      integer ik, ik_max, ik_max2, ikmax
      parameter(ikmax=3000)
      real*8  k, pklin, zred, dz, growth, sigmav2_running
      real*8  G1d, G1t, pkcorr_G2_dd, pkcorr_G2_dt, pkcorr_G2_tt
      real*8  G1d_reg, G1t_reg, exp_fact
      real*8  ak(ikmax), pk(ikmax)
      real*8  ak_RegPT(ikmax), pk_dd(ikmax), pk_dt(ikmax), pk_tt(ikmax)
      common /pk_data/ ak, pk, ik_max
      common /pk_RegPT_data/ ak_RegPT, pk_dd, pk_dt, pk_tt, ik_max2
c     -------------------------------------------
c
      ik_max2 = ik_max
      dz = growth(zred)
c
      open(9,file='pk_RegPT_1loop.dat',status='unknown')
c
      do ik=1, ik_max
         ak_RegPT(ik) = ak(ik)
         k = ak(ik)
c
         call one_loop_Gamma1(1, k, G1d)
         call one_loop_Gamma1(2, k, G1t)
         call calc_pkcorr_G2_tree_tree(1, 1, k, pkcorr_G2_dd)
         call calc_pkcorr_G2_tree_tree(1, 2, k, pkcorr_G2_dt)
         call calc_pkcorr_G2_tree_tree(2, 2, k, pkcorr_G2_tt)
         call calc_running_sigmav2(k, sigmav2_running)
         call find_pk(k, pklin)
c
         exp_fact =  0.5d0 * (k*dz)**2 * sigmav2_running
c
         if(exp_fact.ge.80.d0) then
c
            pk_dd(ik) = 0.d0
            pk_dt(ik) = 0.d0
            pk_tt(ik) = 0.d0
...
c
            G1d_reg = dz*(1.d0 + exp_fact + dz**2*G1d)*dexp(-exp_fact)
            G1t_reg = dz*(1.d0 + exp_fact + dz**2*G1t)*dexp(-exp_fact)
c     
            pk_dd(ik) = G1d_reg**2 * pklin 
     &           + pkcorr_G2_dd * dz**4 * dexp(-2.d0*exp_fact)
            pk_dt(ik) = G1d_reg*G1t_reg * pklin 
     &           + pkcorr_G2_dt * dz**4 * dexp(-2.d0*exp_fact)
            pk_tt(ik) = G1t_reg**2 * pklin 
     &           + pkcorr_G2_tt * dz**4 * dexp(-2.d0*exp_fact)
c
         endif
c
         write(9,'(1p5e18.10)') ak(ik), dz**2*pklin, pk_dd(ik), 
     &        pk_dt(ik), pk_tt(ik)
c
      enddo
c
...
c
      ...
      parameter(ikmax=3000)
      parameter(ixmax=400, imu_max=15)
      real*8  ak(ikmax), pk(ikmax)
      ...
      real*8  q, kq, pk_q, pk_kq, Gamma2_tree
      common  /pk_data/ ak, pk, ik_max
      pi = 4.d0 * datan(1.d0)
c     -----------------------------------------------------
c
...
c
     ...
     &           * Gamma2_tree(a,kq,q,k)*Gamma2_tree(b,kq,q,k)
         enddo
c
...
c ******************************************************* c
c
      subroutine find_pk_RegPT(kk, pkdt, pktt)
c
c ******************************************************* c
c
      implicit none
      ...
      parameter(ikmax=3000)
      real*8  ak(ikmax), pk_dd(ikmax), pk_dt(ikmax), pk_tt(ikmax)
      real*8  kk, s, ds, pkdt, pktt, kmin, kmax
      common /pk_RegPT_data/ ak, pk_dd, pk_dt, pk_tt, ik_max
c     -------------------------------------------
c
      kmin = ak(1)
...
c
      ...
      integer ip
      real*8 fp
      real*8 mu, k, x, pk1_dt, pk1_tt, pk2_dt, pk2_tt
...
c     --------------------------------------- c
c
      call find_pk_RegPT(k*sqrt(1.+x*x-2.*mu*x), pk1_dt, pk1_tt)
      call find_pk_RegPT(k*x, pk2_dt, pk2_tt)
cc      call find_pk(k*sqrt(1.+x*x-2.*mu*x), pk1_dt)
cc      pk1_dt = pk1_dt * dd2
cc      pk1_tt = pk1_dt
cc      call find_pk(k*x, pk2_dt)
cc      pk2_dt = pk2_dt * dd2
cc      pk2_tt = pk2_dt
c
      if(ip.eq.1) then
         fp = x**2 * (mu*mu-1.) / 2. * pk1_dt * pk2_dt
      elseif(ip.eq.2) then
         fp = 3.*x**2 * (mu*mu-1.)**2 / 8. * pk1_dt * pk2_tt
      elseif(ip.eq.3) then
...
!      open(11,file='pkRegPT_Bterm.dat',status='unknown')
...
cc     &        fact(1,4), fact(2,4), fact(3,4), fact(4,4)
     ...
     &        pk_B111(ik),  
     &        ( pk_B112(ik) + pk_B121(ik) ), 
     &        pk_B122(ik), 
     &        pk_B211(ik), 
     &        ( pk_B212(ik) + pk_B221(ik) ),
     &        pk_B222(ik),
     &        ( pk_B312(ik) + pk_B321(ik) ),
     &        pk_B322(ik),
     &        pk_B422(ik)
c
      enddo
...
c
      ...
      real*8 omegab, omega0, omega_m, h, Tcmb, n_s
