...
c                                                         c  
c        --- IR-safe integration implemented ---          c
c                                                         c  
c            Time-stamp: <2016-12-01 09:22:20 ataruya>    c
c ******************************************************* c
c
c     Note--. 
c
...
c     the same. 
c
c     Exponential damping of the propagators is taken into account 
c     (see subroutine integ_A_tA). 
c
      program A_term_II
c
      implicit none
...
c     /////// Save output data ///////
c
      open(10,file='corr_pkred2_II_IRsafe.dat',status='unknown')
      open(11,file='pkcorr2_red_RegPT_II_IRsafe.dat',status='unknown')
c
      do ik =1, ik_max
         write(10,'(1p8e18.10)') ak(ik), pk(ik), 
     &        pk0EH(ik), pk0corr(ik), pk2EH(ik), 
     &        pk2corr(ik), pk4EH(ik), pk4corr(ik) 
         write(11,'(1p4e18.10)') ak(ik), pk_A1(ik), pk_A2(ik), 
     &        pk_A3(ik)
      enddo
c
      write(6,*)
      write(6,*) ' Output file: corr_pkred2.dat, pkcorr2_red.dat'
c
      close(10)
      close(11)
c
...
c
      ...
      real*8  int_A(6), int_tA(6), zred, dd2, growth
c     ---------------------------------------------------
      integer ndim5, ncomp, mineval, maxeval
      integer verbose, last
      double precision epsrel, epsabs
      parameter (ndim5 = 5)
      parameter (ncomp = 12)
      parameter (epsrel = 1D-6)
      parameter (epsabs = 1D-13)
      parameter (verbose = 0)
      parameter (last = 4)
      parameter (mineval = 0)
ccc      parameter (maxeval=9000000)
      parameter (maxeval=3000000)
      integer nnew
      double precision flatness
ccc      parameter (nnew=6000000)
      parameter (nnew=2000000)
      parameter (flatness=5D0)
      integer key
      parameter (key = 0)
...
c
         ...
         pk_tA322(ik)=0.d0
c     
         if(ak(ik).lt.0.01d0) goto 10 ! skipping time-consuming integrals
c
         xmin = kmin / k
         xmax = kmax / k
c
c     ////// Monte Carlo integration //////  c
c
         call suave(ndim5, ncomp, integ_A_tA,
     &        epsrel, epsabs, verbose + last, mineval, maxeval,
     &        nnew, flatness, 
     &        nregions, neval, fail, integral, error, prob)
c
         pk_A111(ik) = 2.d0 * integral(1) * (k*dd2)**3 / (2.*pi)**2
         pk_A121(ik) = 2.d0 * integral(2) * (k*dd2)**3 / (2.*pi)**2
         pk_A221(ik) = 2.d0 * integral(3) * (k*dd2)**3 / (2.*pi)**2
         pk_A212(ik) = 2.d0 * integral(4) * (k*dd2)**3 / (2.*pi)**2
         pk_A222(ik) = 2.d0 * integral(5) * (k*dd2)**3 / (2.*pi)**2
         pk_A322(ik) = 2.d0 * integral(6) * (k*dd2)**3 / (2.*pi)**2
         pk_tA111(ik) = 2.d0 * integral(7) * (k*dd2)**3 / (2.*pi)**2
         pk_tA121(ik) = 2.d0 * integral(8) * (k*dd2)**3 / (2.*pi)**2
         pk_tA221(ik) = 2.d0 * integral(9) * (k*dd2)**3 / (2.*pi)**2
         pk_tA212(ik) = 2.d0 * integral(10) * (k*dd2)**3 / (2.*pi)**2
         pk_tA222(ik) = 2.d0 * integral(11) * (k*dd2)**3 / (2.*pi)**2
         pk_tA322(ik) = 2.d0 * integral(12) * (k*dd2)**3 / (2.*pi)**2
c
...
c ******************************************************* c
c
      subroutine integ_A_tA(ndim, xx, ncomp, int_A_tA)
cc      subroutine integ_A_tA(x, int_A, int_tA)
c
c ******************************************************* c
c
c     int_A_tA(1):  A111,     int_A_tA(7):   tA111
c     int_A_tA(2):  A121,     int_A_tA(8):   tA121
c     int_A_tA(3):  A221,     int_A_tA(9):   tA221
c     int_A_tA(4):  A212,     int_A_tA(10):  tA212
c     int_A_tA(5):  A222,     int_A_tA(11):  tA222
c     int_A_tA(6):  A322,     int_A_tA(12):  tA322
c
      implicit none
      integer ndim, ncomp
      double precision  xx(*), int_A_tA(*)
      double precision  pi
      parameter (pi = 3.14159265358979323846D0) 
c --------------------------------------- c
      integer ip
      real*8  xmin, xmax, kmin, kmax
      real*8  integ_fp_A(6), integ_fp_tA(6)
      real*8  k, x, mu, p, q, kp, theta, phi, jacobian
      real*8  kk(3), pp(3), qq(3)
      real*8  b211A_II,  b221A_II,  b212A_II,  b222A_II
      real*8  b211tA_II, b221tA_II, b212tA_II, b222tA_II
      real*8  b211A_III,  b221A_III,  b212A_III,  b222A_III
      real*8  b211tA_III, b221tA_III, b212tA_III, b222tA_III
...
c
      ...
      kmax = k * xmax
c
      jacobian =  (dlog(kmax)-dlog(kmin))**2 * 2.d0*pi*pi * 2.d0
c
      p = dexp(dlog(kmin) + (dlog(kmax) - dlog(kmin)) * xx(1))
      x = p / k
      mu = -1.d0 + 2.d0 * xx(2)
c
      pp(1) = p * dsqrt(1.d0 -mu*mu)
      pp(2) = 0.d0
      pp(3) = p * mu
c
      q = dexp(dlog(kmin) + (dlog(kmax) - dlog(kmin)) * xx(3))
      theta = xx(4) * pi
      phi = xx(5) * 2.d0 * pi
c
      qq(1) = q * dsin(theta) * dcos(phi)
      qq(2) = q * dsin(theta) * dsin(phi)
      qq(3) = q * dcos(theta)
c
      kk(1) = 0.d0
      kk(2) = 0.d0
      kk(3) = k
c
c     domain of mu-integral is  -1 < mu < min[1/(2x), 1] 
      if(mu.ge.0.5d0/x) then
         do ip=1, 12
            int_A_tA(ip) = 0.d0
         enddo
         goto 1
...
c
      call fp_A(x, mu, integ_fp_A, integ_fp_tA)
      call Bispec_II_IRsafe(pp, kk-pp, -kk, qq, 2, 1, 1, 
     &     b211A_II, b211A_III)
      call Bispec_II_IRsafe(pp, kk-pp, -kk, qq, 2, 2, 1, 
     &     b221A_II, b221A_III)
      call Bispec_II_IRsafe(pp, kk-pp, -kk, qq, 2, 1, 2, 
     &     b212A_II, b212A_III)
      call Bispec_II_IRsafe(pp, kk-pp, -kk, qq, 2, 2, 2, 
     &     b222A_II, b222A_III)
      call Bispec_II_IRsafe(kk-pp, pp, -kk, qq, 2, 1, 1, 
     &     b211tA_II, b211tA_III)
      call Bispec_II_IRsafe(kk-pp, pp, -kk, qq, 2, 2, 1, 
     &     b221tA_II, b221tA_III)
      call Bispec_II_IRsafe(kk-pp, pp, -kk, qq, 2, 1, 2, 
     &     b212tA_II, b212tA_III)
      call Bispec_II_IRsafe(kk-pp, pp, -kk, qq, 2, 2, 2, 
     &     b222tA_II, b222tA_III)
c
      do ip=1, 6
         int_A_tA(ip)   = integ_fp_A(ip) * dexp(-exp_fact) 
     &        * x * q**3 * dsin(theta) * jacobian
         int_A_tA(ip+6) = integ_fp_tA(ip) * dexp(-exp_fact) 
     &        * x * q**3 * dsin(theta) * jacobian
      enddo
c
      int_A_tA(1) = int_A_tA(1) * (b211A_II + b211A_III)
      int_A_tA(2) = int_A_tA(2) * (b221A_II + b221A_III)
      int_A_tA(3) = int_A_tA(3) * (b221A_II + b221A_III)
      int_A_tA(4) = int_A_tA(4) * (b212A_II + b212A_III)
      int_A_tA(5) = int_A_tA(5) * (b222A_II + b222A_III)
      int_A_tA(6) = int_A_tA(6) * (b222A_II + b222A_III)
c
      int_A_tA(7) = int_A_tA(7) * (b211tA_II + b211tA_III)
      int_A_tA(8) = int_A_tA(8) * (b221tA_II + b221tA_III)
      int_A_tA(9) = int_A_tA(9) * (b221tA_II + b221tA_III)
      int_A_tA(10) = int_A_tA(10) * (b212tA_II + b212tA_III)
      int_A_tA(11) = int_A_tA(11) * (b222tA_II + b222tA_III)
      int_A_tA(12) = int_A_tA(12) * (b222tA_II + b222tA_III)
c
 1    continue
c
      end
...
c
!      open(11,file='pkRegPT_Aterm_II.dat',status='unknown')
